...
（`pack/docs/` 僅供 Otto 維護者／寫 skill 時對照 consumer 文件結構，**不是**發行物，**不**經本 skill 同步。）

本 skill 負責對應到 consumer，例如 Cursor → `.cursor/skills|rules|commands`，以及 consumer 根目錄 `AGENTS.md`。

**不做**：改 `apps/`、`packages/`、`docs/`（含產品規格與開發文件）、或任何不在 `manifest.json` → `ownedRoots` 的路徑。也不碰 Otto 上游根目錄的維護用 `AGENTS.md` / `README.md`。
...
| Upstream 實體路徑 | `pack/AGENTS.md`、`pack/skills/`、`pack/rules/`、`pack/commands/`（不含 `pack/docs/`） |
